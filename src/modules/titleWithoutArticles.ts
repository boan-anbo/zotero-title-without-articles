...
function getArticlesFromPreferences(): string[] {
  try {
    ...
    const articlesString = Zotero.Prefs.get(`${addon.data.config.prefsPrefix}.articles`, true) as string;
 ...
 */
export function updateExtraWithSortTitle(extra: string, sortTitle: string): string {
  const fieldName = getSortTitleFieldName();
 ...
 */
export async function processItem(item: any, forceProcess: boolean = false): Promise<boolean> {
  try {
 ...
 */
export async function processItems(items: any[], forceProcess: boolean = false): Promise<number> {
  let processed = 0;
...
export async function clearSortTitle(item: any): Promise<boolean> {
  try {
    ...
    Zotero.logError(new Error(`Failed to clear sort title for item ${item.id}: ${error}`));
    return false;
  }
}
